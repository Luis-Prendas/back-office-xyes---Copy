...
export default function ComponentsAppsDepositReview() {
    const normalBaseUrlAuth = getEnviroment() + '/auth';
    const [pagination, setPagination] = useState({ pageIndex: 0, pageSize: 20 });
    const [statusError, setMessageStatus] = useState(false);

    const fullRoute = `${normalBaseUrlAuth}/api/v1/deposit-review`;
    const routePath = 'deposit-review';
    ...
    const columns = columnsTabSelected(7);
