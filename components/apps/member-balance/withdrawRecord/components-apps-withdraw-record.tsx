...
export default function ComponentsAppsWithdrawRecord() {
    const normalBaseUrlAuth = getEnviroment() + '/auth';
    const [pagination, setPagination] = useState({ pageIndex: 0, pageSize: 20 });
    const [statusError, setMessageStatus] = useState(false);

    const fullRoute = `${normalBaseUrlAuth}/api/v1/withdrawal-record`;
    const routePath = 'withdrawal-record';
    ...
    const columns = columnsTabSelected(10);
